...
| 阶段 2 热键多动作路由工业化 | 部分提前完成 | 冲突检测已落地；配置化与持久化并入设置页一起做（§5.2） |
| 阶段 3 系统语音链路工业化 | 部分提前完成 | 取消语义、Escape、ASR/TTS 互斥已落地；delegate 与 HUD 合并为一步（§5.3） |
| 阶段 4 云端后端 | 未开始 | 前置决策已解除；改为"先接常用几家，再调研补漏" |
| 阶段 5 打磨与发布收尾 | 未开始 | |
...
**保留一项，但理由是功能而非隐私**：`IsSecureEventInputEnabled()` 仍作为 **Copy 路径的前置检查**。系统处于安全输入状态时会直接丢弃合成按键事件，Cmd-C 根本发不出去；留着它是为了立刻回一个明确错误，而不是让用户等满 300 ms 超时再得到一个无法解释的失败。它不再表达"这段文字敏感"。
...
| 云端 Provider 顺序 | **2026-08-12 改**：先接产品已选定的几家常用供应商（逐个接入），常用的都支持后再调研遗漏。阿里云 DashScope（Qwen TTS / CosyVoice 系列）仍是候选之一，但不再是由调研简报冻结的唯一首发 |
| 首版语音参数 | **2026-08-12 改**：语速 + 音量 + 音调（`pitchMultiplier` 系统语音原生支持，成本极低故提前）；中英混排自动分段音色紧随首版之后（§5.2） |
...
前置：§5.2 设置页与 §5.3 HUD 完成（云端 Provider 需要挂在设置页的 Provider 下拉里）。选区门禁 `GO` 仍是**对外宣布功能可用**的前提，但不阻塞云端接入开发。

每接一家的工作量：确认协议与输出编码 → 实现 `CloudTtsProvider` → 设置页追加该 Provider 的配置分组 → 试听打通。第一家还要额外把 `SpeechPlayback`（rodio）与分段机制建起来。

任务：`CloudBackend`（`CloudTtsProvider` + `SpeechPlayback`）与共享配置；按句分段与 5000 字符上限；流式格式原型确认后定 rodio 播放方案（渐进或整段）；连通性测试命令；取消、超时、标准化错误；设置页的供应商告知文案。
...
1. **删除敏感度机制**（§3.4）。清掉 `Sensitivity` 枚举、`sensitivity()`、`SAFE_ROLES`、`SelectionOutcome` 与 `TtsRequest` 的对应字段、元素级 `AXSecureTextField` 拒读；保留 `IsSecureEventInputEnabled()` 作为 Copy 路径的功能性前置检查。这一步不管后面怎么排都要做，且能顺带简化数据模型。
2. **朗读设置页**（§5.2）。设置数据模型 → `/reading-settings` 路由与侧边栏 → 页面内容。热键配置化（阶段 2 剩余项）在这一步一并完成。
3. **delegate + HUD**（§5.3 与阶段 3）。两者合并实施：`AVSpeechSynthesizerDelegate` 既修掉极短文本误报 `start_timeout`，又提供驱动 HUD 波形所需的 `willSpeakRange` 事件。
4. **中英分段音色**（§5.2 末尾）。需要一个语言分段器，独立一块工作。
5. **云端 Provider 逐个接入**（阶段 4）。先接常用的几家，再调研补漏。
...
**始终欠着的一项**：人工清单 #5（听写回归 smoke，约 2 分钟）。热键路由已改动且第 2 步还会再改，**建议在第 2 步开工前先做一次**，否则之后出问题分不清是哪一轮引入的。之所以不自动化：需要注入听写热键、录真实音频、发真实 ASR 请求，并把转写注入到当时的前台应用——副作用超出 smoke 应有的范围。
