...
| **朗读引擎** | 系统语音（离线、免配置）、火山引擎豆包 Seed-TTS 2.0，或阿里云百炼（`qwen3-tts-flash`，48 个音色，含北京、上海、四川、粤语等方言；`qwen-audio-3.0-tts-flash`，单次可读更长文本）。两家云端引擎都是流式合成，按下热键后约 0.4–0.6 秒开始出声。每个引擎的音色、语速、音量独立设置 |
